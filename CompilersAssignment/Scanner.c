...
*************************************************************/
/*
Team work by Harsh Bansal and Arshdeep Kaur for chandigarh language*/
/*************************************************************
* File name: Scanner.c
...
chandigarh_int line;									/* Current line number of the source code */
extern chandigarh_int errorNumber;					/* Defined in platy_st.c - run-time error number */

extern chandigarh_int stateType[];
extern chandigarh_chr* keywordTable[];
extern PTR_ACCFUN finalStateTable[];
extern chandigarh_int transitionTable[][TABLE_COLUMNS];
...
chandigarh_int startScanner(BufferPointer psc_buf) {
	/*printf("Harsh");*/
	if (isEmpty(psc_buf)==CHANDIGARH_TRUE)
...
Token tokenizer(void) {
	...
	chandigarh_chr c;	/* input symbol */
	chandigarh_int state = 0;		/* initial state of the FSM */
	chandigarh_int lexStart;		/* start offset of a lexeme in the input char buffer (array) */
	chandigarh_int lexEnd;		/* end offset of a lexeme in the input char buffer (array)*/

	chandigarh_int lexLength;		/* token length */
	chandigarh_int i;				/* counter */
	chandigarh_chr newc;			/* new char */
	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		*/
		...
		case '+':
			...
			return currentToken;
			/*to also include the - sign for the negative integers???????*/
		case '-':
			currentToken.code = ART_OP_T;
		...
		/*case '_':
		/*	currentToken.code = MNID_T; return currentToken;*/
		case ',':
		...
		case '|':
			...
			if (newc == '>') {
				...
				} while (flag == 0 /*flag ==1 */ && c != CHARSEOF0 && c != CHARSEOF255);
			}
			else if (newc == '|') {
				currentToken.code = LOG_OP_T;
				currentToken.attribute.codeType = OP_OR;
...
chandigarh_int nextState(chandigarh_int state, chandigarh_chr c) {
	chandigarh_int col;
	chandigarh_int next;
	col = nextClass(c);
...
chandigarh_int nextClass(chandigarh_chr c) {
	chandigarh_int val = -1;
	/* Adjust the logic to return next column in TT */
	/*	[A-z](0), [0-9](1),	_(2), .(3), #(4), %(5), $(6), &(7), "(8), SEOF(9), other(10) */
	/*	[A-z](0), [0-9](1),	-(2), .(3), "(4), '(5), _(6), SEOF(7), O(8) */
	switch (c) {
	case CHRCOL2:
		val = 2;
		break;
...
Token funcIL(chandigarh_chr lexeme[]) {
	...
	if (lexeme[0] != '\0' && strlen(lexeme) > NUM_LEN) {
		...
		if (tlong >= 0 && tlong <= SHRT_MAX) {
			...
			currentToken.attribute.intValue = (chandigarh_int)tlong;
		}
		/*for the neagtive integers*/
		else if (tlong >= 0 && tlong <= SHRT_MAX && lexeme[0] == '-') {
			currentToken.code = INL_T;
			currentToken.attribute.intValue = (chandigarh_int)tlong;
		}
		else {
...
Token funcFPL(chandigarh_chr lexeme[]) {
	Token currentToken = { 0 };
	chandigarh_dbl tdouble = atof(lexeme);
	if (tdouble == 0.0 || tdouble >= FLT_MIN && tdouble <= FLT_MAX) {
		currentToken.code = FPL_T;
		currentToken.attribute.floatValue = (chandigarh_flt)tdouble;
	}
	else if (lexeme[0] == '-' && tdouble == 0.0 || tdouble >= FLT_MIN && tdouble <= FLT_MAX) {
		currentToken.code = FPL_T;
		currentToken.attribute.floatValue = (chandigarh_flt)tdouble;
	}
	else {
...
Token funcSL(chandigarh_chr lexeme[]) {
	Token currentToken = { 0 };
	chandigarh_int i = 0, len = (chandigarh_int)strlen(lexeme);
...
chandigarh_nul printToken(Token t) {
	extern chandigarh_chr* keywordTable[]; /* link to keyword table in */
	switch (t.code) {
	case RTE_T:
 ...
 *		Function responsible to handle the char literals
 * It is only taking the value that is between the single quotes ignoring the 
 * single quotes and returning the error if the length of the char is greater than 3 means two 
 * quotes and a char
 ************************************************************/
...
Token funcCL(chandigarh_chr lexeme[]) {
	...
	/*chandigarh_int i = 0 ,len = (chandigarh_int)strlen(lexeme);*/
	chandigarh_int len = (chandigarh_int)strlen(lexeme);
	if (len == 3) {
		if (lexeme[0] == '\'' && lexeme[2] == '\'') {
			if (lexeme[1] != CHARSEOF0) {
				lexeme[0] = '\0';
				lexeme[2] = '\0';
				currentToken.attribute.contentChar[0] = lexeme[1];
				/*strncpy(currentToken.attribute.contentChar, lexeme[1], CHAR_LEN);*/
	...
	/*strncpy(currentToken.attribute.contentChar, lexeme, CHAR_LEN);
	/*currentToken.attribute.idLexeme[3] = CHARSEOF0;*/
